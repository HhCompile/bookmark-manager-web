 ...
 * @param props - 组件属性
 */
export default function SmartTaggingResult({
  taggingResult,
}: SmartTaggingResultProps) {
  // 如果没有标记结果，不渲染任何内容
  if (!taggingResult) {
    return null;
  }
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {taggingResult.categories.map((category, index) => (
              <div
                key={index}
                className="bg-gray-50/50 rounded-lg p-3 border border-border"
              >
        ...
        <div className="mt-6">
          ...
          <div className="flex flex-wrap gap-2">
            {taggingResult.tags.map((tag, index) => (
              ...
              >
                {tag.name}{' '}
                <span className="ml-1 text-primary/70">{tag.count}</span>
              </span>
            ))}
          </div>
        </div>
